...
export default function ListingCard({ listing }: { listing: any }) {
  const img = (listing?.images && listing.images[0]) || 'https://placehold.co/800x450?text=Boat';
  return (
    <Link
      href={`/listing/${listing.id}`}
      className="rounded-xl overflow-hidden border border-white/10 bg-white/5 hover:bg-white/10 transition"
    >
      <img src={img} alt={listing.title} className="w-full aspect-video object-cover" />
